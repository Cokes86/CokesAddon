...
public class BoxerR extends CokesAbility implements TargetHandler {
    ...
    private static final Config<Double> STRAIGHT_DAMAGE_PERCENTAGE = new Config<>(BoxerR.class, "straight-damage-percentage", 110.0,
            PredicateUnit.positive(),
            "# 스트레이트 검 비례 대미지", "# 기본값: 110.0 (%)");
    private static final Config<Double> COUNTER_DAMAGE_PERCENTAGE = new Config<>(BoxerR.class, "counter-damage-percentage", 80.0,
            PredicateUnit.positive(),
    ...
    private static final Config<Double> COUNTER_THORN_PERCENTAGE = new Config<>(BoxerR.class, "counter-thorn-percentage", 50.0,
            ...
            "# 카운터 반사 대미지", "# 기본값: 50.0 (%)");
    private static final Config<Double> UPPER_DAMAGE_PERCENTAGE = new Config<>(BoxerR.class, "upper-damage-percentage", 90.0,
            PredicateUnit.positive(),
    ...
    public void TargetSkill(Material material, LivingEntity livingEntity) {
        if (swords.contains(material) && skillTimer.isRunning() && livingEntity.equals(skillTimer.participant.getPlayer()) && straight_cooldown.isCooldown()) {
    ...
    public void onPlayerSwapHandItems(PlayerSwapHandItemsEvent e) {
        if (e.getOffHandItem() != null && swords.contains(e.getOffHandItem().getType()) && e.getPlayer().equals(getPlayer())) {
            ...
            if (skillTimer.isRunning() && counter_cooldown.isCooldown() && entity.equals(skillTimer.participant.getPlayer())) {
    ...
    public void onPlayerDropItem(PlayerDropItemEvent e) {
        if (swords.contains(e.getItemDrop().getItemStack().getType()) && e.getPlayer().equals(getPlayer())) {
            ...
            if (skillTimer.isRunning() && upper_cooldown.isCooldown() && entity.equals(skillTimer.participant.getPlayer())) {
                float sword = SwordDamage.valueOf(sword_material.name()).getDamage();
                int sharpness = sword_itemstack.getEnchantmentLevel(Enchantment.DAMAGE_ALL);
    ...
    public void onPlayerToggleSneak(PlayerToggleSneakEvent e) {
        if (e.getPlayer().equals(getPlayer()) && e.isSneaking()) {
            if (skillTimer.isRunning() && ducking_cooldown.isCooldown()) {
