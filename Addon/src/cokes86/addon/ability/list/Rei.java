...
@AbilityManifest(name = "레이", rank = Rank.S, species = Species.HUMAN, explain = {
		...
		"자신의 체력이 0이 되었을 시 그 공격을 무효로 하고 체력이 $[respawn]이 됩니다. $[cool]",
		"체력이 2이하인 상태에서는 코스트를 소비하지 않습니다.",
		"※제작자 자캐 기반 능력자"
})
			...
			if (e.getDamager().equals(getPlayer())) {
				if (!c.isRunning()) {
					...
					if (getPlayer().getHealth() > 2) {
						getPlayer().setHealth(Math.max(0.0, getPlayer().getHealth() - getPlayer().getAttribute(Attribute.GENERIC_MAX_HEALTH).getBaseValue() * cost.getValue() / 100.00));
					}
				}
