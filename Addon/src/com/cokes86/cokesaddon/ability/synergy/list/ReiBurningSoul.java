...
@AbilityManifest(name = "레이<버닝소울>", rank = Rank.S, species = Species.DEMIGOD, explain = {
        "§7패시브 §8- §c타오르는 §f: 체력 회복량이 2배가 되지만, 항상 불타는 상태입니다.",
        "  받는 화염 피해가 (§c버닝 소울§f × $[FIRE_DAMAGE_INCREMENT])% 증가합니다.",
        "§7적 처치 §8- §c수거§f: §c버닝 소울§f을 하나 얻습니다. 최대 $[MAX_SOUL]개까지 보유 가능합니다.",
        "§7근거리 공격 §8- §c한 맺힌 힘§f: 대상을 §7(§e1 + §c버닝 소울§7)§f초간 추가로 불태웁니다.",
        "  만약, 대상이 이미 $[DAMAGE_INCREMENT_PREDICATE] 이상 발화중이라면, 추가로 발화하지 않고",
        "  남은 발화시간에 비례하여 추가 피해를 준 후 불을 끕니다.",
        "  §7추가 피해량: §e초과한 시간(틱) × 0.06",
        "§7철괴 우클릭 §8- §c영혼 해방§f: 자신의 최대 체력의 $[SOUL_LIBERATION_COST]%을 지불하고,",
        ...
        "  §c영혼 해방§f으로 얻은 §c버닝 소울§f은 §c수거§f의 §c버닝 소울§f의 제약에 속하지 않습니다.",
...
public class ReiBurningSoul extends CokesSynergy implements ActiveHandler {
    ...
    private static final Config<Integer> DAMAGE_INCREMENT_PREDICATE = Config.time(ReiBurningSoul.class, "damage-increment-predicate", 5,
    ...
    private static final Config<Integer> TEMPORARY_SOUL_DURATION = Config.time(ReiBurningSoul.class, "temporary-soul-duration", 7,
    ...
    private static final Config<Integer> LIBERATION_SOUL_FIRE_DURATION = Config.time(ReiBurningSoul.class, "liveration-soul-fire-duration", 2,
    ...
    private static final Config<Integer> LAST_BURNING_FIRE_DURATION = Config.time(ReiBurningSoul.class, "last-burning-fire-duration", 5,
