...
@AbilityManifest(
		...
		explain = {"게임 중 최초로 철괴 우클릭 시 범위 $[r]블럭 이내 모든 범위를 자경단 아지트로 만듭니다.",
...
public class VigilanteLeader extends AbilityBase implements ActiveHandler {
	...
	Timer passive = new Timer() {
		...
		protected void run(int count) {
			if (ajit != null) {
				for (Location l : Circle.iteratorOf(ajit, r.getValue(), r.getValue()*8).iterable()) {
					l.setY(LocationUtil.getFloorYAt(l.getWorld(), l.getY(), l.getBlockX(), l.getBlockZ())+0.1);
					for (Participant p : vigilantes) ParticleLib.REDSTONE.spawnParticle(p.getPlayer(), l, new RGB(0,162,232));
					ParticleLib.REDSTONE.spawnParticle(getPlayer(), l, new RGB(0,162,232));
				}
				
				ArrayList<Player> inAjit = LocationUtil.getNearbyPlayers(ajit, r.getValue(), r.getValue());
				addNum = inAjit.size();
				
				if (inAjit.contains(getPlayer())) {addNum +=2;} else {addNum += 1;}
				for (Participant vigilante : vigilantes) {
					if (inAjit.contains(vigilante.getPlayer())) addNum+=1;
				}
	...
	public boolean ActiveSkill(Material m, ClickType ct) {
		if (m.equals(Material.IRON_INGOT) && ct.equals(ClickType.RIGHT_CLICK)) {
			if (ajit == null) {
				...
				if (!c.isCooldown()) {
					getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, (getGame().getParticipants().size()+addNum)*10, 0));
