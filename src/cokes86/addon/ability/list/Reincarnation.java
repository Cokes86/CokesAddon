...
public class Reincarnation extends AbilityBase {
	...
	public static Config<Integer> 
	duration = new Config<Integer>(Reincarnation.class, "지속시간", 20, 2) {
	...
	public void onEntityDamage(EntityDamageEvent e) {
		if (e.getEntity().equals(getPlayer())) {
			...
			else if (!reincarnation.isRunning() && getPlayer().getHealth() - e.getFinalDamage() <= 0 && !cool.isRunning()) {
				e.setDamage(0);
	...
	Timer reincarnation = new Timer(duration.getValue()*20) {
		...
		protected void run(int arg0) {
			getPlayer().setHealth(1);
			ac.update("지속 시간: "+TimeUtil.parseTimeAsString(getFixedCount())+ " 히트횟수: "+ (hitted >= hit.getValue() ? "§a"+hitted : hitted) + "/ "+hit.getValue());
